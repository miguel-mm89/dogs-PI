...
export default function Pagination({
 ...
 if(currentPage === pageNumbers.length && currentPage+1 > 5){
  ...
  return (
    ...
    <PaginationContainer>
      <NextPrev>
      <button disabled={currentPage === 1? true: false} className={currentPage === 1? 'disable': null} onClick={() => dispatch(paginate(currentPage -1))}><GrPrevious/></button>
      </NextPrev>
      <NumbersDiv>
      {pageShow?.map(item => <div className={currentPage === item? "active": null} key={item}>
        <button  onClick={() => dispatch(paginate(item)) }>{item}</button>
      </div> 
      )}
      </NumbersDiv>
      <NextPrev>
      <button className={currentPage === pageNumbers.length? 'disable': null} disabled={currentPage === pageNumbers.length? true: false} onClick={() => dispatch(paginate(currentPage +1))}><GrNext/></button>
      </NextPrev>
    </PaginationContainer>
</>
  );
